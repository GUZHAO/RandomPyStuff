...
# keep long urls on one line for readability
"""
# This script prints out users by Tableau Server group by site
#
# To run the script, you must have installed Python 3,
...
def query_groups(server, auth_token, site_id, page_size, page_number):
    ...
    if page_size == 0:
        ...
        url = server + "/api/{0}/sites/{1}/groups?pageSize={2}&pageNumber={3}".format(VERSION, site_id, page_size,
                                                                                      page_number)
...
def get_users_in_group(server, auth_token, site_id, group_id, page_size, page_number):
    ...
    if page_size == 0:
        ...
        url = server + "/api/{0}/sites/{1}/groups/{2}/users?pageSize={3}&pageNumber={4}".format(VERSION, site_id,
                                                                                                group_id, page_size,
                                                                                                page_number)
